...
export async function POST(req: NextRequest) {
  ...
  for (const item of cart.items) {
    ...
    } else if (Number(item.quantity) > Number(item.product.stock)) {
      stockIssues.push(`Stok ${item.product.name} tersisa ${Number(item.product.stock)}`);
    }
  }
  ...
  const order = await prisma.$transaction(async (tx) => {
    ...
    for (const { item } of lineItems) {
      // stock sekarang Decimal, jadi dipotong sesuai kuantitas ASLI (mis. 0.25 kg
      // beneran cuma motong 0.25, bukan dibulatkan ke 1 seperti sebelumnya).
      await tx.product.update({
        where: { id: item.productId },
        data: {
          stock: { decrement: Number(item.quantity) },
          soldCount: { increment: Math.ceil(Number(item.quantity)) }, // soldCount tetap Int, hitung "berapa kali laku"
        },
      });
    }
  ...
  try {
    const transaction = await createPaymentTransaction({
      ...
      user: {
        name: userRecord?.name ?? "Pelanggan",
      ...
      for (const { item } of lineItems) {
        await tx.product.update({
          ...
          data: {
            ...
            soldCount: { decrement: Math.ceil(Number(item.quantity)) },
          },
        });
      }
    });
